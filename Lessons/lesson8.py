...
# list collection
# ordered collection tha can be changable and allow duplicates
# and uses square brackets to define the collected dsata

colors = ['red', 'blue', 'green', 'purple', 'magenta', 'black',
          'white', 'orange']
size = [42, 32,256, 89, 34, 23, 8, 19]
...
# using in keyword to reseach a list for the item
def simple_search():
    if 89 in size:
...
# append ans insert to a list
# append will inswert at the end of te list
def simple_append():
    mixed.append('Friday')
    for mix in mixed:
        print(mix)
...
# removing items from a list by removing(), pop(), del and clear()
# the remove function will remove from specified value form the list
def simple_remove():
    colors.remove('green')
    for color in colors:
        print(color)
...
# the pop method will removw an item from a specific index.
#not picked an index will take the last item
def simple_pop():
    colors.pop(5)
    for value in colors:
        print(value)
...
# list operations: Concatention, repeat, membership, and iterate
# concatention
def simple_join():
    values1 = [1, 2, 3, 4]
    values2 = [5, 6, 7, 8]
...
# my_fruits = ('apple', 'banana', 'chery', 'orange')
...
# change a value in a tuple, but converting to list then back again
...
# prt 2 of collections with set and dictionary
# set collection
# the order of the items vary
# ce_cream = {'chocolate', 'vanilla', 'strawberry', 'neopolitan',
...
# remove will generate an error if the oitem being removed does not exist
...
# dicard wil remove withou generating an error
...
# pop will remove the last item from the list so you dont konw what it is
def simple_set_pop():
    letters = {'a', 'b', 'c', 'd', 'e', 'f'}
    letters.pop()
...
# the key is uniqu and can not be duplicate making immutable
# te value however can be a duplicate. Defined using curky braces
...
my_pallet = {'color': {'primary': 'red', 'secondary': 'maroon'},
             'colors2': {'primary': 'blue', 'secondary': 'royal'}}
